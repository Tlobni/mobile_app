...
class NotificationService {
  ...
  static void handleNotification(RemoteMessage? message, bool isTerminated, [BuildContext? context]) {
    ...
    if (notificationType == "chat" && !isTerminated) {
      ...
      if (senderId == currentlyChatingWith && itemId == currentlyChatItemId) {
        ...
        if (userType == "Buyer") {
          (context)?.read<GetSellerChatListCubit>().addOrUpdateChat(ChatUser(
              ...
              updatedAt: date,
              item: Item(id: int.parse(itemId), price: getPrice((itemPrice)), name: itemName, image: itemImage),
  ...
  static Future<void> foregroundNotificationHandler(BuildContext context) async {
  ...
  static void onTapNotificationHandler(context) {
    onMessageOpen = FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
      ...
      } else if (message.data["item_id"] != null && message.data["item_id"] != '') {
